...
whsqrt_t = tf.sqrt(tf.maximum(wh_t,1e-3))
box_num = tf.maximum(tf.reduce_sum(mask_box),1.)
coordxy_t = (xy_t+tf.expand_dims(tf.stack(tf.meshgrid(tf.range(th),tf.range(th),indexing='xy'),axis=-1),axis=2))/th#[None,7,7,2,2]
...
detector_out = tf.layers.conv2d(tmp,30,(1,1),kernel_initializer=tf.truncated_normal_initializer(stddev=0.001))/50#[None,7,7,30]
...
whsqrt = tf.sqrt(tf.maximum(wh,1e-3))#[None,7,7,2,2]
iou_p = tf.reshape(detector_out[...,8:10],tf.concat([ts,[2]],axis=0))+0.5#[None,7,7,2]
cls = tf.nn.softmax(detector_out[...,10:30])#[None,7,7,20]

coordxy = (xy+tf.expand_dims(tf.stack(tf.meshgrid(tf.range(th),tf.range(th),indexing='xy'),axis=-1),axis=2))/th#[None,7,7,2,2]
...
union = tf.maximum(tf.multiply(*tf.unstack(rt-lf,axis=-1))+tf.multiply(*tf.unstack(rt_t-lf_t,axis=-1))-sect,1e-3)#[None,7,7,2]
...
iouerr = tf.reduce_mean((iou_p-iou_t)**2 * tf.where(tf.equal(resp_mask,1.),resp_mask*5,tf.ones_like(resp_mask)*0.5))
